...
pub use archival_new::{ArchivalManager, Passage, ArchivalSearchResult};
pub use compaction::{CompactionManager, SummaryResult};
pub use context::ContextManager;
pub use db::{MemoryDb, preference_keys, PreferenceDb};
pub use embedding::EmbeddingService;
pub use tools::{
    MemoryAppendTool, MemoryReplaceTool, MemoryInsertTool,
    ConversationSearchTool, ArchivalInsertTool, ArchivalSearchTool,
    SetPreferenceTool,
};
...
impl MemoryManager {
    ...
    pub fn get_recent_messages(&self, limit: usize) -> Result<Vec<(String, String, chrono::DateTime<chrono::Utc>)>> {
        let messages = self.recall.get_recent(limit)?;
        Ok(messages.into_iter().map(|m| (m.role, m.content, m.created_at)).collect())
    }
    ...
    pub fn compile_metadata(&self) -> String {
        ...
        s.push_str(&format!("- The current system date is: {}\n", 
            chrono::Local::now().format("%Y-%m-%d %H:%M:%S %Z")));
        
        if let Some(modified) = last_modified {
            s.push_str(&format!("- Memory blocks were last modified: {}\n", 
    ...
    pub fn get_preference(&self, key: &str) -> Result<Option<String>> {
        Ok(self.db.preferences().get(self.agent_id, key)?.map(|p| p.value))
    }
    ...
    pub fn get_timezone(&self) -> Result<Option<chrono_tz::Tz>> {
        if let Some(tz_str) = self.get_preference(preference_keys::TIMEZONE)? {
            Ok(Some(tz_str.parse::<chrono_tz::Tz>()
                .map_err(|_| anyhow::anyhow!("Invalid timezone stored: {}", tz_str))?))
        } else {
            Ok(None)
        }
    }
    ...
    pub fn get_context_messages(&self) -> Result<(Option<SummaryRow>, Vec<MessageRow>)> {
        ...
        let messages = if let Some(ref s) = summary {
            ...
            if after_summary.len() < MIN_MESSAGES_IN_CONTEXT {
                self.db.messages().get_recent(self.agent_id, MIN_MESSAGES_IN_CONTEXT as i64)?
            } else {
                after_summary
            }
        } else {
    ...
    pub async fn store_message_with_compaction_check(
        ...
        let compacted = if self.compaction.should_compact(current_tokens, DEFAULT_CONTEXT_WINDOW, COMPACTION_THRESHOLD) {
            tracing::info!(
    ...
    pub async fn run_compaction(&self) -> Result<SummaryResult> {
        ...
        let summary_boundary = current_summary.as_ref().map(|s| s.to_sequence_id).unwrap_or(0);
        ...
        if to_summarize_count == 0 {
            anyhow::bail!("Not enough messages to compact (need to keep {} minimum)", MIN_MESSAGES_IN_CONTEXT);
        }
    ...
    fn estimate_context_tokens(&self, summary: &Option<SummaryRow>, messages: &[MessageRow]) -> usize {
        ...
        let message_chars: usize = messages.iter().map(|m| m.content.len() + m.role.len() + 10).sum();
        (summary_chars + message_chars) / 4
    }
    
    /// Search summaries by semantic similarity
    pub async fn search_summaries(&self, query: &str, limit: usize) -> Result<Vec<db::SummarySearchResult>> {
