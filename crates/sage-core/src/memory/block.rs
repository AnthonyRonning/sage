...
impl BlockManager {
    ...
    pub fn new(agent_id: Uuid, db: MemoryDb) -> Result<Self> {
        ...
        if db_blocks.is_empty() {
            info!(
                "No existing blocks found, creating defaults for agent {}",
                agent_id
            );
            ...
            info!(
                "Loaded {} blocks from database for agent {}",
                db_blocks.len(),
                agent_id
            );
...
mod tests {
    ...
    fn test_block_replace() {
        let agent_id = Uuid::new_v4();
        let mut block = Block::new(agent_id, "test").with_value("Hello, world!");
    ...
    fn test_block_append() {
        let agent_id = Uuid::new_v4();
        let mut block = Block::new(agent_id, "test").with_value("Line 1");
