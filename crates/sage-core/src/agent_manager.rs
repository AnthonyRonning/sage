...
impl AgentManager {
    ...
    pub async fn get_or_create_agent(
        ...
        // Create new agent
        info!("Creating new agent for {} (id: {})", signal_identifier, agent_id);
        ...
        // Cache it
        {
            ...
            agents.insert(agent_id, CachedAgent {
                agent: agent.clone(),
                context,
            });
        }
    ...
    fn get_or_create_context(
        ...
        if let Some(ctx) = existing {
            debug!("Found existing context for {}: {}", signal_identifier, ctx.id);
            return Ok(ctx);
        }
        
        // Create new context
        let new_id = Uuid::new_v4();
        info!("Creating new chat context for {} -> {}", signal_identifier, new_id);
    ...
    async fn create_agent(&self, agent_id: Uuid) -> Result<SageAgent> {
        ...
        // Configure LLM
        SageAgent::configure_lm(&self.maple_api_url, &self.maple_api_key, &self.maple_model).await?;
