...
use crate::scheduler::{
    is_cron_expression, next_cron_time, parse_cron, parse_datetime,
    MessagePayload, SchedulerDb, TaskPayload, TaskStatus, TaskType, ToolCallPayload,
};
...
impl Tool for ScheduleTaskTool {
    ...
    async fn execute(&self, args: &HashMap<String, String>) -> Result<ToolResult> {
        // Parse task_type
        let task_type_str = args.get("task_type")
            .ok_or_else(|| anyhow::anyhow!("'task_type' argument required (message or tool_call)"))?;
        ...
        let run_at = args.get("run_at")
            .ok_or_else(|| anyhow::anyhow!("'run_at' argument required (ISO datetime or cron expression)"))?;
        ...
        let (next_run_at, cron_expression): (DateTime<Utc>, Option<String>) = 
        ...
        // Parse payload
        let payload_str = args.get("payload")
...
impl Tool for ListSchedulesTool {
    ...
    async fn execute(&self, args: &HashMap<String, String>) -> Result<ToolResult> {
        ...
        match self.scheduler_db.get_tasks_by_agent(self.agent_id, status_filter) {
            Ok(tasks) => {
                if tasks.is_empty() {
